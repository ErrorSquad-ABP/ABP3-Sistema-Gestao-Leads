...
| `SEED_MODE` | Não | `minimal` (padrão): utilizadores + equipas + lojas. `dashboard`: dataset completo a partir do CSV em `prisma/data/`. |
| `SEED_DEFAULT_PASSWORD` | Não | Palavra-passe comum aos utilizadores do seed `minimal` (padrão alinhado ao CSV: `admin123`). |

## Seed do banco (`prisma/seed.ts`)

1. Suba o PostgreSQL (ex.: `docker compose up -d postgres` na raiz do repositório) e aplique migrations (`npm run db:migrate` na raiz, ou `prisma migrate dev` no `back`).
2. Na raiz: `npm run db:seed` (equivale a `npm run prisma:seed -w back`).
3. Por omissão usa `SEED_MODE=minimal`: cinco utilizadores (todos os `UserRole`), duas equipas e duas lojas, com IDs determinísticos. Para o cenário analítico completo: `SEED_MODE=dashboard`.
